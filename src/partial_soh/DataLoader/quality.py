...
# 因此它们的容量尖峰循环当时没有登记到坏循环表。切换到 123 口径后这两只
# 被保留（Severson 124 名单里没有删它们），所以这里必须补登记，否则
# soh_nominal 会出现 1.40 / 2.62 这类物理上不可能的尖峰值。
BAD_CYCLES: dict[str, frozenset[int]] = {
    "20170512_c000": frozenset({12}),       # 容量尖峰（q_max 约 1.54）
    "20170512_c018": frozenset({40}),       # 容量尖峰（q_max 约 2.88）
