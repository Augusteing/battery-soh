...
针对 SIT 数据集，我们做了三项与 Severson 不同的数据标定处理：
...
**SOH 标签口径**。SIT 的 SOH 标签采用"当前循环充电容量 / 该电池最大充电容量"（Qc / Qc_max），与 Severson 的"充电容量 / 标称容量 1.1 Ah"口径不同——原因是 SIT 为 50 Ah 方形电芯，充电容量可达 52~55 Ah、没有统一的标称容量参考，以电池自身最大充电容量为参考更贴近 BMS 的实际标定方式。两套口径已在 4.1.1 说明，本实验的所有结论均在同一口径内比较。

**分组与车辆微调协议**。20 只 SIT 电池分为环境温组（Repower_001 与 Chroma_101，共 10 只，片段温度约 25~58°C 波动）与 40°C 恒温箱组（Repower_002 与 Repower_003，共 10 只，温度稳定在约 39~47°C），每只电池包含数百至近千个循环。车辆级实验中，用每只电池前 300 个循环微调、之后循环测试，并过滤 SOH < 0.75 的循环（对齐 BMS 正常工作区间）。片段切分（20% 容量窗口、最多 51 个起点、101 个等容量点）与 12 维温度形状特征提取与 Severson 完全一致，输入经跨电芯归一化（电流 → C-rate、容量坐标 → SOC），保证 1.1 Ah 与 50 Ah 电芯共用同一模型。
