...
> 状态：草稿。所有数字必须与 `results/metrics/` 及运行日志核对后才可定稿。
> 数字来源标记：✅ = 已落盘/有记录；⏳ = 正在运行；TBD = 待补。
...
| 数据集 | 电池 | 规格 | 温度 | 角色 |
| --- | --- | --- | --- | --- |
| Severson（MATR） | 138（剔除坏电池后） | 1.1 Ah 18650 LFP | 30°C 恒温 | 阶段一预训练 |
| SIT | 20 | 50 Ah 方形 LFP | 环境温（001/101）+ 40°C 恒温箱（002/003） | 阶段二跨电芯/车辆微调 |

### 5.1.2 评估协议与指标

- **协议一（同分布）**：Severson 按电池划分 110 训练 / 14 验证 / 14 测试，检验预训练模型在其数据分布内的精度；
...
| --- | --- | --- | --- |
| 3 通道（I, V, Q） | 0.00027 | **1.40% ✅** | 最终使用的预训练模型（README 8/28 记录） |
