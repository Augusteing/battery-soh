...
| 4 通道（+dV/dQ） | 0.000111 | **1.21%** | 2026-08-31 一阶段全量重训（见 5.3.1） |

结论：模型在其训练分布内已具备良好精度（~1.5%）。其中**温度曲线通道**在恒温 Severson 上无增益（1.63% vs 1.47%），佐证"温度模块应只在变温微调阶段启用"的设计；而**物理导数通道 dV/dQ** 带来明确增益（1.21%，见 5.3.1），说明恒温数据下也有可挖掘的曲线形变信息。
...
### 5.3.1 dV/dQ 第 4 物理通道：相变峰预处理（导师方案验证）

LFP 电池老化时，差分电压 dV/dQ（增量容量）曲线上的相变特征峰会随 SOH 衰减发生位置偏移与展宽，即使平台区绝对电压几乎不变。我们最初尝试**显式峰特征**方案：提取片段窗口内 tanh(dV/dQ) 序列的均值/方差/偏度三阶矩，拼入温度特征向量。该方案在 B1 跨温度组上反而变差（5.02% → 5.65%）。诊断表明矩特征与 SOH 的相关很弱（组内 |r| ≤ 0.15），却与温度组高度判别（组间 t ≈ ±95），在跨温度场景重新引入了"绝对温度 = 电池身份"的捷径。

随后按导师建议改为**通道式输入**：在等容量网格上直接用 Savitzky–Golay 滤波（窗口 21 点、3 阶多项式、`deriv=1`、步长 0.002 SOC）输出解析导数 dV/dQ，经 tanh 压制到 [-1, 1] 后作为第 4 物理通道 (I, V, Q, dV/dQ) 送入编码器，让 LSTM 在时间步展开中学习导数随容量网格的流动。该管线在 Severson 123 只电池上全量重训一阶段（50 个预训练 epoch + 50 个 SOH 微调 epoch，全程解冻），预训练 loss 0.00027 → 0.000111，微调 loss 收敛至 0.000053。

**Severson 同分布测试**（86.9 万未见片段）：测试 MAE 从 3 通道基线的 1.47% 降至 **1.21%**（−0.26pp）。

**SIT 跨电芯/跨温度验证**（片段级 MAE，与旧 3 通道管线同测试集同划分对比）：

| 实验 | 3 通道基线 | 4 通道 dV/dQ | 变化 |
| :--- | :--- | :--- | :--- |
| A：同分布跨电芯（14 训练 / 6 测试） | 4.66% | **4.24%** | −0.42pp |
| B1：跨温度外推，无温度模块 | 5.54% | **4.55%** | **−0.99pp** |
| B1：跨温度外推，FiLM+相对温度 | 5.02% | 4.96% | −0.06pp |

三个结论：

第一，**通道式 dV/dQ 全面优于矩特征方案**，也全面优于 3 通道基线。最有力的证据是 B1 跨温度、无温度口径下的 −0.99pp：该对照不含任何温度模块，唯一差异是 dV/dQ 通道（含 4 通道重训），证明导数信息本身显著改善跨温度泛化。

第二，**温度模块在 4 通道管线里的定位需要重新审视**。B1 上"无温度版"（4.55%）反超"FiLM+相对温度版"（4.96%）0.41pp，与 3 通道管线中的方向（FiLM 优于无温度 0.52pp）相反。合理的解释是：dV/dQ 通道本身已编码温度诱导的电压形变，FiLM 的温度特征变得冗余甚至轻微干扰。温度模块可能应简化为仅特定场景（如环境温组内大温度摆动）启用。

第三，边界与口径。以上均为 seed=42 单次运行，B1-FiLM 组的 −0.06pp 在噪声范围内，不作为证据；"通道效应"包含 4 通道输入与 4 通道重训两部分，如需进一步拆分可做"4 通道预训练、微调时第 4 通道置零"的消融。

实现位置：`scripts/convert_cache_dvdq.py`（缓存第 4 通道转换）、`src/temperature_soh/Trainer/finetune_sit.py`（`--dvdq-channel`）、模型 `models/temperature_soh/dvdq4ch/pretrain_dvdq_4ch.pt`。
